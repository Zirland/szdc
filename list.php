...
switch ($t) {
	...
	case 'ro' :
		...
		$query32 = "SELECT route FROM `ignore`;";
		...
		echo "</tr>";
		$query = "SELECT * FROM route WHERE (active = 1) ORDER BY agency_id DESC, route_short_name;"; // AND route_color = 'B51741' AND route_id NOT LIKE 'L%'
		if ($result = mysqli_query ($link, $query)) {
			while ($row = mysqli_fetch_row ($result)) {
				$route_id = $row[0];
				$agency_id = $row[1];
				...
				echo "<td";
				if ($route_active == "1") {
					if (in_array($route_id, $ignorelist)) {
						echo " style=\"background-color: #F5B708;\"";
					} else {
